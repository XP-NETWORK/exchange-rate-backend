...
app.get("/batch_data", async (_, res: Response<BatchDataResp>) => {
    const cache = await cmcCachedExchange.getBatchedRate(Object.values(SupportedCurrency));
    const entries = Object.fromEntries(cache) as BatchDataResp;

    res.set('Cache-Control', `public, max-age=${cmcCachedExchange.getCacheExpiry()/1000}`)
    res.send(entries);
});
